 ...
 */
export const ProjectSection = memo(function ProjectSection({ project }: { project: Project }) {
  const [open, setOpen] = useState(false);
  const setTargets = useScan((s) => s.setTargets);

  const selectable = useMemo(
    () => project.targets.filter((t) => t.risk !== "protected"),
    [project.targets],
  );
  const chosenCount = useScan(
    (s) => selectable.reduce((n, t) => n + (s.selected.has(t.id) ? 1 : 0), 0),
  );
  const all = selectable.length > 0 && chosenCount === selectable.length;
  const some = chosenCount > 0 && !all;
